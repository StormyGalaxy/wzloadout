...
describe('generateGithubLink', () => {
  ...
  it('Correctly uses default owner', () => {
    const link = generateGithubLink(undefined, 'testRepo', { title: 'Test Title' });
    expect(link).toBe('https://github.com/SiloCityLabs/testRepo/issues/new?title=Test+Title');
  });

  it('Correctly handles an empty repo', () => {
    const link = generateGithubLink('testOwner', '', { title: 'Test Title' });
    expect(link).toBe('https://github.com/testOwner//issues/new?title=Test+Title');
  });
  ...
  it('Correctly handles an empty array for labels or assignees', () => {
    ...
    expect(link).toBe('https://github.com/testOwner/testRepo/issues/new?title=Test+Title');
  });
});

            ...
            <Form onSubmit={handleSubmit}>
                ...
                {githubAssignees.length > 0 && (
                    <Form.Group className="mb-3" controlId="assignees">
                        ...
                        <Select<OptionType, true>
                            isMulti options={githubAssignees} value={formData.assignees}
                            onChange={(opts) => handleChangeSelect(opts, 'assignees')}
                            closeMenuOnSelect={false} placeholder="None" aria-label="Select Assignees"
                        />
                    </Form.Group>
                )}
                ...
                {githubLabels.length > 0 && (
                    <Form.Group className="mb-3" controlId="labels">
                        ...
                        <Select<OptionType, true>
                            isMulti options={githubLabels} value={formData.labels}
                            onChange={(opts) => handleChangeSelect(opts, 'labels')}
                            closeMenuOnSelect={false} placeholder="None" aria-label="Select Labels"
                        />
                    </Form.Group>
                )}
                ...
                {githubTemplates.length > 0 && (
                    <Form.Group className="mb-3" controlId="template">
                        ...
                        <Form.Select name="template" value={formData.template || ''} onChange={handleInputChange} aria-label="Select Template">
                ...
                {formData.template === '' && (
                    <Form.Group className="mb-3" controlId="body">
                        ...
                        <Form.Control as="textarea" name="body" value={formData.body} onChange={handleInputChange} rows={5} aria-label="Feedback Description" />
                    </Form.Group>
                )}

                <div className="text-center mt-4 d-flex justify-content-center">
                    <Link href="/" passHref legacyBehavior>
                        <Button variant="outline-secondary" className="me-3">Cancel</Button>
                    </Link>
                    <Button variant="primary" type="submit">Create GitHub Issue</Button>
                </div>
            </Form>
        </div>
    );

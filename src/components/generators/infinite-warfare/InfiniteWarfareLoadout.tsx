...
export default function InfiniteWarfareLoadout() {
  ...
  const { randClassName, perkObj, streaks, weapons, equipment, wildcards, combat_rig } = data;
...
async function fetchLoadoutData(setData) {
  ...
  try {
    ...
    if (loadoutFrame.primary_optic) {
      weapons.primary.optic = fetchAttachments(weapons.primary.weapon, 'optic')[0];
    }

    //Get Primary Attachments
    if (!weapons.primary.weapon?.no_attach && loadoutFrame?.primary_attach > 0) {
      weapons.primary.attachments = implodeObject(
        fetchAttachments(weapons.primary.weapon, 'attachments', loadoutFrame.primary_attach)
      );
    }
    ...
    if (!weapons.secondary.weapon?.no_attach && loadoutFrame.secondary_optic) {
      weapons.secondary.optic = fetchAttachments(weapons.secondary.weapon, 'optic')[0];
    }

    //Verify if secondary weapon has attachments
    if (!weapons.secondary.weapon?.no_attach && loadoutFrame.secondary_attach > 0) {
      weapons.secondary.attachments = implodeObject(
        fetchAttachments(weapons.secondary.weapon, 'attachments', loadoutFrame.secondary_attach)
      );
    }
    ...
    const equipment = {
      ...
      lethal: loadoutFrame.lethal ? fetchEquipment('lethal', game) : { name: '', type: '' },
    };
    ...
    setData({ randClassName, perkObj, streaks, weapons, equipment, wildcards, combat_rig });
  } catch (error: unknown) {
